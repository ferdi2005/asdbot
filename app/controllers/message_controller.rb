...
class MessageController < ActionController::API
  def message_process
    ...
    text = if message[:text].nil?
             message[:caption]
           else
             message[:text]
        end
    if text.split('@').count == 2
      text = text.split('@')[0] if text.split('@')[1] == ENV['BOT_USERNAME']
    end
    ...
    if Group.find_by(chat_id: id) && (type == 'group' || type == 'supergroup')
    ...
    unless @group.nil?
      ...
      @admins['result'].each do |result|
        next if result.nil?
        next if result['user'].nil?

        next if result['user']['username'].nil?
    ...
    if Sender.find_by(chat_id: fromid)
